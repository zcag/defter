...
export function App() {
  ...
  const charts = useMemo(() => {
    const m = parse(text)
    const computed = engine.compute(m)
    const out: { key: string; type: any; title?: string; labels: string[]; series: number[][]; seriesNames: string[] }[] = []
    ...
    <section className="features" id="why">
      ...
      <div className="caps">
        {[
          ...
          'cross-sheet refs',
          'VLOOKUP / INDEX-MATCH',
          'live formula-ref highlighting',
          'point-mode ref insert',
