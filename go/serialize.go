...
func serializeSheet(sheet *Sheet, forceHeading bool) string {
	...
	if len(sheet.Styles) > 0 || len(sheet.Charts) > 0 || len(sheet.Conditionals) > 0 ||
		len(sheet.Validations) > 0 || len(sheet.Checkboxes) > 0 || len(sheet.Dates) > 0 ||
		len(sheet.Names) > 0 || sheet.Freeze.isSet() {
		block := serializeStyleBlock(sheet.Styles, sheet.Charts, sheet.Conditionals, sheet.Validations, sheet.Checkboxes, sheet.Dates, sheet.Names, sheet.Freeze)
		parts = append(parts, "\n```defter-style\n"+block+"\n```")
	}
